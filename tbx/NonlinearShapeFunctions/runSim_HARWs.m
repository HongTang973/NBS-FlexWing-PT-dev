function SimObject = runSim_HARWs(t1,t2,varargin)
...
% **** if this run is restarted from existing obj, a function handle can be
% inherited *** 
if isprop(SimObject, 'QOI_Master') && ~isempty(SimObject.QOI_Master)
    fHandle = SimObject.QOI_Master.fHandle;
else
    % assign the fHandle with specified function script
...
yp0      = fHandle(0,SimObject.IC, SimObject,'dQ');
[y0,yp0] = decic(fHandle, 0, SimObject.IC,[],yp0, [],  options, SimObject, 'dQ-imp', 'fAE_type','imp');
options = odeset('OutputFcn',@outputFunction,'BDF','off','relTol',1e-5);
[t,u] = ode15i(fHandle, tsteps, y0, yp0, options, SimObject,'dQ-imp','fAE_type','imp');
...
profile on;
SimObject.initialise_QOI_Master('fHandle',fHandle);
