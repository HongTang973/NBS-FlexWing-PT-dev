...
if ~isempty(aerodynamics)
    ...
    switch aerodynamics
        ...
        %TODO pre allocate aero matrices after the first iteration
            ...
            omr = - utility_functions.crossn(Omega_G,Gamma_G_pm_global,1);
            ph = SimObject.flexParts_nonlinear.O_turbineBlade_1.ph0(end)*180/pi;
            vfs = omr(1,:,end) * tand(10-ph) * uVec_freeStream_G;
            Vinf = omr + vfs;
            ...
            if ~SimType.Parked
                ...
                for aeroPartName_cell = aeroPartNames
                    ...
                    r = sqrt(Gamma_G_pm_part(1,:,:).^2 ...
                        + Gamma_G_pm_part(2,:,:).^2 ...
                        + Gamma_G_pm_part(3,:,:).^2);

                    r_ = sqrt(Gamma_G_pn_part(1,:,end).^2 ...
                        + Gamma_G_pn_part(2,:,end).^2 ...
                        + Gamma_G_pn_part(3,:,end).^2);

                    r_tip = (r_ - r)./r;
                    r_hub = (r - SimType.r_hub)./r;
...
%                             cl_global = ds;
                        ...
                        case 'oye' %1 state per strip (dynamic only in stall)
                            ...
                            [dQaero, cl_global, cd_global, cm_global] = aero.dynamicStall.oye(Qaero, alpha_global, oyeCoeff);
                ...
                case 'lookup 2D'
                    ...
                    if SimType.Parked
                        AICs_global = ones(2,1,nsAp);
                        r = sqrt(Gamma_G_pm_global(1,:,:).^2 ...
                            + Gamma_G_pm_global(2,:,:).^2 ...
                            + Gamma_G_pm_global(3,:,:).^2);

                        r_ = sqrt(Gamma_G_pn_global(1,:,end).^2 ...
                            + Gamma_G_pn_global(2,:,end).^2 ...
                            + Gamma_G_pn_global(3,:,end).^2);

                        r_tip = (r_ - r)./r;
                        r_hub = (r - SimType.r_hub)./r;

                        f = 3.*[r_tip; r_hub]./abs(sa)./2;
                        rm = real(abs(f))<7;
                        AICs_global(rm) = 2*acos(exp(-f(rm)))/pi;
                        AICs_global = AICs_global(1,:,:).*AICs_global(2,:,:);
                    else
                ...
                case 'leishman'
                    ...
                    %                     f = 3.*(obj.L - s_aeroMid_global)./(s_aeroMid_global.*sin(alpha_global(end)))./2;
                    % %                     f = f.*3.*(s_aeroMid_global - SimType.r_hub)./(s_aeroMid_global.*sin(alpha_global(end)))./2;
                    %                     AICs_global = real(2.*acos(exp(-f))./pi);
...
%    output = dW_dqg_static_sum;
    ...
    if ~isempty(SimObject.aeroPartNames) && ~isempty(aerodynamics)
        ...
        PvecAero_A_pm_global = utility_functions.MultiProd_(R_G_A.',PvecAero_G_pm_global);
        ...
        MvecAero_A_pm_global = utility_functions.MultiProd_(R_G_A.',MvecAero_G_pm_global);
